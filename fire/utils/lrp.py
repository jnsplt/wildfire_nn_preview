...
def relprop(a, layer, R, gamma=0., eps=0, debug=False, extra=False):
    ...
    Issues: #todo
        [ ] Copy layer before applying it, to avoid errors due to inplace=True
    """
    if debug==True:
    ...
    # Why is eps a constant value here? Setting zero for now
  #  eps = 0.
        
    a = a.data.requires_grad_(True) # copy as in tut., otherwise backward (below) fails
    z = eps + rho.forward(a) 
...
def init_relevances(activations: List[torch.Tensor], max_only: bool=False, 
